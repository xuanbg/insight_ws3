...
select C.ID, C.ObjectId, PS.ProductName, PS.ProductSpeci, isnull(PS.ProductPrice, 0) as ProductPrice, isnull(O.OrderAmount, 0) as OrderAmount, isnull(O.FirstPay, 0) as FirstPay, isnull(O.StageAmount, 0) as StageAmount, PS.ProductImage, O.StagePlan, C.CreateTime, 
