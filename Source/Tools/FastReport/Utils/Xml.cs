...
/*        while (Items.Count > 0)
    ...
    {
      return GetProp(index, true);
    }

    internal string GetProp(string index, bool convertFromXml)
    {
      string result = "";
      int i = FText.IndexOf(index + "=\"", StringComparison.OrdinalIgnoreCase);
      if (i != -1 && (i == 0 || FText[i - 1] == 32))
      {
        int start = i + index.Length + 2;
        int quotepos = FText.IndexOf("\"", start);
        result = FText.Substring(start, quotepos - start);
        if (convertFromXml)
          result = Converter.FromXml(result);
      }
      else
        result = "";
      return result;
      ...
      do
      {
        ...
        else if (state == ReadState.FindRight)
        {
          ...
          else
          {
            ...
            i++;
            if (i == 3 && builder[0] == '!' && builder[1] == '-' && builder[2] == '-')
            {
