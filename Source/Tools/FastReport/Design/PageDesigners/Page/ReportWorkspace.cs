...
namespace FastReport.Design.PageDesigners.Page
{
  internal class ReportWorkspace : UserControl
  {
    ...
    private void CreateItemTitle(string text, Column column)
    {
      bool changed = false;
      
      // try to create title for the inserted item
      ComponentBase insertedObject = Designer.SelectedObjects[0] as ComponentBase;
    ...
    protected override void OnMouseMove(MouseEventArgs e)
    {
      ...
      else if (FMode2 == WorkspaceMode2.Move || FMode2 == WorkspaceMode2.Size)
      {
        ...
        if (FMode1 == WorkspaceMode1.DragDrop)
        {
          ...
          {
            ComponentBase c = Designer.Objects[i] as ComponentBase;
            if (c != null && c != FEventArgs.DragSource && !c.HasRestriction(Restrictions.DontModify))
          ...
          FEventArgs.DragSource.SetFlags(Flags.CanDraw, !dragHandled);
        }
        ...
        if (FMode1 == WorkspaceMode1.DragDrop && !dragHandled)
        {
          ...
          FEventArgs.DragSource.Left = (int)Math.Round(FEventArgs.DragSource.Left / Grid.SnapSize) * Grid.SnapSize;
          FEventArgs.DragSource.Top = (int)Math.Round(FEventArgs.DragSource.Top / Grid.SnapSize) * Grid.SnapSize;
        }
      ...
      if (FMode2 == WorkspaceMode2.Move || FMode2 == WorkspaceMode2.Size)
      {
        ...
        if (FMode1 == WorkspaceMode1.DragDrop)
        {
          if (FEventArgs.DragTarget != null)
          {
            ...
            Designer.Objects.Remove(FEventArgs.DragSource);
            FEventArgs.DragSource.Dispose();
            FEventArgs.DragSource = null;
            Designer.SelectedObjects.Clear();
    ...
    protected override void OnDragOver(DragEventArgs drgevent)
    {
      ...
      if (FMode1 != WorkspaceMode1.DragDrop)
      {
        ...
        ObjectInfo info = RegisteredObjects.FindObject(objType);
        Designer.InsertObject(info, InsertFrom.Dictionary);
        ComponentBase obj = Designer.SelectedObjects[0] as ComponentBase;
        if (obj is TextObject)
        {
    ...
    protected override void OnDragDrop(DragEventArgs drgevent)
    {
      ...
      if (FEventArgs.DragSource != null)
        CreateItemTitle(item.Text, item.Obj as Column);
    }
