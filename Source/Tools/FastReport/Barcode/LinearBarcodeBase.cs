...
namespace FastReport.Barcode
{
  internal enum BarLineType
  {
    White,
    Black,
    // a black line with 2/5 height (used for PostNet)
    BlackHalf,
  ...
  public class LinearBarcodeBase : BarcodeBase
  {
    ...
    private void DoLines(string data, Graphics g, float zoom)
    {
      using (Pen pen = new Pen(Color.Black))
      {
        ...
        foreach (char c in data)
        {
          ...
          float height = FBarArea.Height;
          if (lt == BarLineType.BlackHalf)
            height = FBarArea.Height * 2 / 5;
          ...
          pen.Width = width;
          if (lt == BarLineType.BlackHalf)
            g.DrawLine(pen, xadd + width / 2, FBarArea.Bottom * zoom, xadd + width / 2, FBarArea.Bottom * zoom - height);
          else if (lt != BarLineType.White)
            g.DrawLine(pen, xadd + width / 2, FBarArea.Top * zoom, xadd + width / 2, FBarArea.Top * zoom + height);
    ...
    internal void DrawString(Graphics g, float x1, float x2, string s, bool small)
    {
      ...
      using (Font drawFont = new Font(font.Name, font.Size * fontZoom, font.Style))
      {
        ...
        size.Width /= Zoom;
        size.Height /= Zoom;
        g.DrawString(s, drawFont, Brushes.Black,
