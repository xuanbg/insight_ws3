        ...
        if (cr.Errors.Count > 0)
        {
          ...
          foreach (CompilerError ce in cr.Errors)
          {
            ...
            if (line == -1)
            {
              ...
              errors += String.Format("({0}): " + Res.Get("Messages,Error") + " {1}: {2}", new object[] { 
                errObjName, ce.ErrorNumber, ce.ErrorText }) + "\r\n";
              if (Report.Designer != null)
                Report.Designer.ErrorMsg(errObjName + ": " + Res.Get("Messages,Error") + " " + ce.ErrorNumber + 
                  ": " + ce.ErrorText, errObjName);
            }
            else
            {
              errors += String.Format("({0},{1}): " + Res.Get("Messages,Error") + " {2}: {3}", new object[] { 
                line, ce.Column, ce.ErrorNumber, ce.ErrorText }) + "\r\n";
              if (Report.Designer != null)
                Report.Designer.ErrorMsg(Res.Get("Messages,Error") + " " + ce.ErrorNumber + ": " + ce.ErrorText,
                  line, ce.Column);
            }
          }
