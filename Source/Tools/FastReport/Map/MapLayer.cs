...
namespace FastReport.Map
{
  ...
  public class MapLayer : Base, IParent, ICustomTypeDescriptor
  {
    ...
    /// Gets or sets 
    /// </summary>
    public string ZoomShape
    {
      get { return FZoomShape; }
      set
      {
        FZoomShape = value;
        foreach (ShapeBase shape in Shapes)
        {
          ...
          {
            ZoomShapePolygon(shape as ShapePolygon);
            break;
          }
        }
      }
    ...
    public void ZoomShapePolygon(ShapePolygon shape)
    {
      ...
      using (Graphics g = Graphics.FromImage(bmp))
      {
        ...
        shape.GetGraphicsPath(new FRPaintEventArgs(g, 1, 1, null)).Dispose();
        RectangleF largestBoundsRect = shape.largestBoundsRect;
