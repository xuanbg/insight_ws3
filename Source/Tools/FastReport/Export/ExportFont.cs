...
namespace FastReport.Export.TTF
{
    ...
    internal class ExportTTFFont : IDisposable
    {
        ...
        public string GetEnglishFontName()
        {
            ...
            if ((FSourceFont.Style & FontStyle.Bold) > 0)
                Style.Append("Bold");
            if ((FSourceFont.Style & FontStyle.Italic) > 0)
                Style.Append("Italic");
            if (Style.Length > 0)
        ...
        private int GetGlyphIndices(IntPtr hdc, string text, ushort[] glyphs, int[] widths, bool rtl)
        {
