...
namespace FastReport.Export
{
    internal static class ExportUtils
    {
      ...
      public static string GetExcelFormatSpecifier(FormatBase format)
      {
        ...
        {
          CurrencyFormat f = format as CurrencyFormat;
          string fm_str = "#,##0";
          if (f.DecimalDigits > 0)
          {
            fm_str += f.DecimalSeparator;
            for (int i = 0; i < f.DecimalDigits; i++)
              fm_str += "0";
          }
        ...
        {
          NumberFormat f = format as NumberFormat;
          string fm_str = "#,##0";
          if (f.DecimalDigits > 0)
          {
            fm_str += f.DecimalSeparator;
        ...
        public static string MD5Stream(Stream stream, long start, long length)
        {
            ...
            int read = stream.Read(buff, 0, (int)length);
            stream.Position = oldpos;
            byte[] hash = MD5buf(buff, read);
            StringBuilder sb = new StringBuilder(32);
            for (int i = 0; i < hash.Length; i++)
                sb.Append(hash[i].ToString("x2"));
            return sb.ToString();
        ...
        {
            return MD5Stream(stream, 0, stream.Length);
        }

        public static string MD5Stream(Stream stream, long length)
        {
            return MD5Stream(stream, 0, length);
        }
        ...
        public static long Crc32(Stream src)
        {
            return Crc32(src, 0, src.Length);
        }

        public static long Crc32(Stream src, long start, long length)
        {
            ...
            long c = 0xffffffff;
            while (src.Position < (start + length))
            {
