...
namespace FastReport.Export.OoXML
{
    ...
    class OoXMLDocument : OoXMLBase
    {
        ...
        { 
            ...
            Out.WriteLine("<w:tblCellMar><w:left w:w=\"0\" w:type=\"dxa\"/><w:right w:w=\"0\" w:type=\"dxa\"/></w:tblCellMar>");
...
//              Out.WriteLine("<wp:effectExtent l="19050" t="0" r="0" b="0" />");
        ...
        private void Export_Paragraph(ExportIEMObject Obj, StreamWriter Out)
        {
            ...
            using (GraphicCache cache = new GraphicCache())
            {
                ...
                foreach (AdvancedTextRenderer.Paragraph paragraph in renderer.Paragraphs)
                {
                    ...
                    FTextStrings.AppendLine("<w:spacing w:after=\"0\" w:line=\"240\" w:lineRule=\"auto\" />" );
                            ...
                            foreach (AdvancedTextRenderer.Word word in line.Words)
                                foreach (AdvancedTextRenderer.Run run in word.Runs)
                                    using (Font fnt = run.GetFont())
                                        Add_Run(fnt, run.Style.Color, run.Text, true, Obj.Style.RTL );
                        }
                    }
                    else
                    {
                        ...
                        foreach (AdvancedTextRenderer.Line line in paragraph.Lines)
                            text += " " + line.Text;
                        Add_Run(f, Obj.Style.TextColor, text, false, Obj.Style.RTL);
        ...
        private void Export_TableCell(ExportIEMObject Obj, int CellWidth, int dx, int dy, StreamWriter Out)
        {
            ...
            if (Obj != null)
            {
                ...
                // Set padding
                Out.WriteLine("<w:tcMar>" +
                    "<w:top w:w=" + Quoted(Obj.Style.Padding.Top * 17) + " w:type=\"dxa\"/>" +
                    "<w:left w:w=" + Quoted(4 + Obj.Style.Padding.Left * 17) + " w:type=\"dxa\"/>" +
                    "<w:bottom w:w=" + Quoted(Obj.Style.Padding.Bottom * 17) + " w:type=\"dxa\"/>" +
                    "<w:right w:w=" + Quoted(4 + Obj.Style.Padding.Right * 17) + " w:type=\"dxa\"/>" +
                    "</w:tcMar>");
            }
                ...
                if (FMatrix != null) // Table based export
                {
                    ...
                    for (int y = 0; y < FMatrix.Height - 1; y++)
                    {
                        if (pbk < FMatrix.PagesCount - 1)
                        {
                            ...
                            {
                                Out.WriteLine("<w:p><w:pPr><w:rPr> <w:sz w:val=\"0\"" +
                        ...
                        if (renderer.HtmlTags)
                        {
                            ...
                            foreach (AdvancedTextRenderer.Line line in paragraph.Lines)
                            {
                                foreach (AdvancedTextRenderer.Word word in line.Words)
                                    foreach (AdvancedTextRenderer.Run run in word.Runs)
                                        using (Font fnt = run.GetFont())
                                            Add_Run(fnt, run.Style.Color, run.Text, true, (reportObject as TextObject).RightToLeft);
                            }
                        }
                        else
                        {
                            ...
                            foreach (AdvancedTextRenderer.Line line in paragraph.Lines)
                                text += " " + line.Text;
                            Add_Run(f, textObject.TextColor, text, false, (reportObject as TextObject).RightToLeft);
                        }
                        Close_Paragraph();
                    }
                }
...
//            Directory.Delete(FTempFolder, true);
        ...
        protected override void Start()
        {
            ...
            Directory.CreateDirectory(FTempFolder + "/word/theme");
