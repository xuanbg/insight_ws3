    ...
    partial class WebReport
    {
        ...
        private void CreateNavigatorControls()
        {
            ...
            if (ShowToolbar)
            {
                ...
                if (ShowXpsExport && Config.FullTrust)
                {
                    item = new ListItem(Res.Get("Export,Xps,File"), "xps");
                    cbbExportList.Items.Add(item);
                }
                ...
                if (ShowWord2007Export && Config.FullTrust)
                {
                    item = new ListItem(Res.Get("Export,Docx,File"), "docx");
                    cbbExportList.Items.Add(item);
                }
                ...
                if (ShowOdtExport/* && Config.FullTrust*/)
                {
                    item = new ListItem(Res.Get("Export,Odt,File"), "odt");
                    cbbExportList.Items.Add(item);
                }
                if (ShowDbfExport)
                {
                    item = new ListItem(Res.Get("Export,Dbf,File"), "dbf");
                    cbbExportList.Items.Add(item);
                }
                if (ShowCsvExport)
                {
                ...
                btnPrint.OnClientClick = String.Format("frxOpenPrint('{0}?{1}={2}&{3}={4}'); return false;", 
                    WebUtils.HandlerFileName,
                    WebUtils.PrintPrefix, 
                    PrintInPdf && Config.FullTrust ? "pdf" : "html",
                    WebUtils.ReportPrefix, 
                    ReportGuid);
            ...
            hiddenID.Name = WebUtils.HiddenIDSuffix;
            hiddenID.ID = WebUtils.HiddenIDSuffix;
            hiddenID.Value = WebUtils.ReverseString(ReportGuid);
        ...
        void btnExport_Click(object sender, ImageClickEventArgs e)
        {
            ...
            switch (cbbExportList.SelectedValue)
            {
                case "csv":
                    ExportCsv();
                    break;
        ...
        {            
            ...
            if (cbbZoom.SelectedValue == "1000")
                z = (float)Math.Round((float)(this.Width.Value - 20) / Properties.CurrentWidth, 2);
            else if (cbbZoom.SelectedValue == "1001")
                z = (float)Math.Round(Math.Min((float)(this.Width.Value - 20) / Properties.CurrentWidth,
                    (float)(this.Height.Value - 40) / Properties.CurrentHeight), 2);
            else
        ...
        private void tbPage_TextChanged(object sender, EventArgs e)
        {
          ...
          try
          {
            pageNo = Convert.ToInt16(tbPage.Text);
          }
          catch
          {
          }
